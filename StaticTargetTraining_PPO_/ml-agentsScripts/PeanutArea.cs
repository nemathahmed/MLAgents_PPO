...
public class PeanutArea : Area
{
    // Start is called before the first frame update
    public GameObject peanutAgent;
    ...
    private void FixedUpdate()
    {
        ...
        Vector3 peanutLocalPosition = peanutAgent.transform.localPosition;
        if (Mathf.Abs(peanutLocalPosition.x) > 13f || Mathf.Abs(peanutLocalPosition.z) > 13f)
        {
            Debug.LogWarning("Peanut out of the pen!");
            PeanutAgent peanutAgentComponent = peanutAgent.GetComponent<PeanutAgent>();
            peanutAgentComponent.SetReward(-5f);
            peanutAgentComponent.AgentReset();
            ResetArea();
        }
    }
    ...
    private void ResetAgent()
    {
        ...
        RandomlyPlaceObject(peanutAgent, spawnRange, 10);
    }
    ...
    */
    ...
    private void RandomlyPlaceObject(GameObject objectToPlace, float range, float maxAttempts)
    {
        ...
        objectToPlace.GetComponent<BoxCollider2D>().enabled = false;
        ...
        {
            Vector3 randomLocalPosition = new Vector3(UnityEngine.Random.Range(-range, range), UnityEngine.Random.Range(-range, range),1);
        ...
        // Enable collision
        objectToPlace.GetComponent<BoxCollider2D>().enabled = true;
    }
